// Pomocné funkcie - Fixed Version 5.5
...
function initBrandGrid(gridId = 'brandGrid') {
    ...
    if (!CONFIG.DEVICE_BRANDS || !Array.isArray(CONFIG.DEVICE_BRANDS)) {
        console.error('CONFIG.DEVICE_BRANDS nie je definované');
        return;
    }
    ...
    appData.categories.forEach(cat => {
        if (cat.diagnoses) {
            cat.diagnoses.forEach(d => {
                const title = (d.translations?.sk || d.translations?.de)?.title || d.id;
                const category = (cat.translations?.sk || cat.translations?.de)?.name || cat.id;
                trees.push({
                    id: d.id,
                    title: title,
                    category: category
