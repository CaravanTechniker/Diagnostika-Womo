...
function deduplicateByKey(items, keyCandidates = []) {
    const seen = new Set();
    return (items || []).filter(item => {
        if (!item || typeof item !== 'object') return false;
        let key = null;
        for (const candidate of keyCandidates) {
            if (item[candidate] !== undefined && item[candidate] !== null && String(item[candidate]).trim() !== '') {
                key = `${candidate}:${String(item[candidate]).trim().toLowerCase()}`;
                break;
            }
        }
        if (!key) {
...
function importErrors(data, action) {
    ...
    if (Array.isArray(data)) {
        if (action === 'replace') {
            appData.errorCodes[brand] = deduplicateByKey(data, ['code', 'id', 'name']);
        } else {
            appData.errorCodes[brand] = deduplicateByKey([...appData.errorCodes[brand], ...data], ['code', 'id', 'name']);
        }
    } else {
        appData.errorCodes[brand] = deduplicateByKey([...appData.errorCodes[brand], data], ['code', 'id', 'name']);
    }
...
function importManual(data, action) {
    ...
    if (Array.isArray(data)) {
        if (action === 'replace') {
            MANUALS_DATA[brand].items = deduplicateByKey(data, ['url', 'title', 'id']);
        } else {
            MANUALS_DATA[brand].items = deduplicateByKey([...MANUALS_DATA[brand].items, ...data], ['url', 'title', 'id']);
        }
    } else {
        MANUALS_DATA[brand].items = deduplicateByKey([...MANUALS_DATA[brand].items, data], ['url', 'title', 'id']);
    }
    ...
    if (action === 'replace') {
        ...
        if (data.categories) {
            data.categories.forEach(newCat => {
                ...
                if (existing) {
                    existing.diagnoses = deduplicateByKey([...(existing.diagnoses || []), ...(newCat.diagnoses || [])], ['id']);
                } else {
